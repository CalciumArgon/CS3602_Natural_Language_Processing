...
class SLUTagging(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.decoder_cell == "FNN":
            ...
            self.output_layer = TaggingRNNDecoder(config.hidden_size, config.num_tags, config.tag_pad_idx, config.decoder_cell)
    ...
    def decode(self, label_vocab, batch):
        ...
        prob = output[0]
        # print("(output[0]) prob.shape:", prob.shape)
        predictions = []
        for i in range(batch_size):
            # print("prob[i] len:", prob[i].shape)
            ...
            for idx, tid in enumerate(pred):
                ...
                if (tag == 'O' or tag.startswith('B')) and len(tag_buff) > 0:
                    ...
                    idx_buff.append(idx)
                    tag_buff.append(tag)
            # print("idx_buff, tag_buff, pred_tags LENGTH:", len(idx_buff), len(tag_buff), len(pred_tags))
            if len(tag_buff) > 0:
                slot = '-'.join(tag_buff[0].split('-')[1:])
                value = ''.join([batch.utt[i][j] for j in idx_buff])
                pred_tuple.append(f'{slot}-{value}')
            predictions.append(pred_tuple)
        # assert False, "one batch"
        if len(output) == 1:
            return predictions
        else:
            loss = output[1]
