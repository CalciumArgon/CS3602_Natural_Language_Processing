...
def make_path(args):
    return (
        f"{args.name}_{date}_max_epoch={args.max_epoch}_bs={args.batch_size}_lr={args.lr}_decay={args.decay_step}_gamma={args.gamma}"
        f"_encoder={args.encoder_cell}_decoder={args.decoder_cell}_dropout={args.dropout}"
    )
...
def visualizer(args, clear_visualizer=True):
    ...
    filewriter_path = os.path.join(root_path, "visualization", save_path)
